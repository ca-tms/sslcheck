 ...
 * 
 * The structure of the CertVerifyRequest-message is - Header - Certificate
 * (DER-encoding) - Server that sent the certificate in the format
 * HostName|HostIP|HostPort
 * 
 * The CertVerifyRequest-class stores this message and additionally the IP that
 * sent the request-message and the IP that received the request-message.
 * 
...
public class CertVerifyRequest extends Message {
	...
	// Additionally it accepts host names containing "_"-characters which seem
	// to be used e.g. by amazonaws.com
	...
	/**
	 * Read a CertVerifyRequest-message from a InputStream. During the reading a
	 * lot of checks on the validity of the supplied data are performed. If one
	 * of them fails an exception is thrown.
	 * 
	 * @param in
	 *            The InputStream to read the CertVerifyRequest from
	 * @param remoteAddr
	 *            The IP address of the client that sent the CertVerifyRequest
	 * @param localAddr
	 *            The IP of the local interface that received the
	 *            CertVerifyRequest
	 * @return A CertVerifyRequest containing the information read from the
	 *         InputStream (i.e. the CertVerifyRequest-message) as well as the
	 *         requesting and the receiving IP.
	 * @throws IOException
	 * @throws CertificateException
	 */
	public static CertVerifyRequest readFromStream(InputStream in,
			String remoteAddr, String localAddr) throws IOException,
		...
		// First Verify the message is actually of type
		// MESSAGE_TYPE_CERT_VERIFY_REQUEST
		int messageType = bin.read();
		if (messageType != Message.MESSAGE_TYPE_CERT_VERIFY_REQUEST) {
			throw new IllegalArgumentException("The provided messageType "
					+ messageType + " was not expected");
		}
		...
		if (bin.read(messageLengthB, 0, 2) != 2) {
			throw new IOException(
					"Reached unexpected end of stream while extracting message length.");
		}
		...
		// Cast the Message's Number-Of-Certificates-In-Chain-field into an
		// integer
		...
		// Read the message's remainder. It should be of the format
		// "HostName|HostIP|HostPort". Therefore it can be split into an array
		// of size three.
		String[] host = Message.readNCharsFromStream(bin,
				messageLength - 5 - certificateChainBytes).split("\\|");

		// Assert that the host-parameter actually consists of three parts.
		if (host.length != 3) {
			throw new IllegalArgumentException(
					"The host-parameter could not be split in three parts.");
		}

		// The Hostname should match the validHostnameRegex and it's length
		// should be something between 3 and 2042 since this is the maximum of
		// the database "Host" column (therefore "HostPort" has a maximum of
		// 2048)
		if (host[0].length() <= 3 || host[0].length() >= 2042
				|| !host[0].matches(validHostnameRegex)) {
			throw new IllegalArgumentException(
					"The provided hostname is not valid: " + host[0]);
		}
		...
		if (!isValidIPAddress(host[1])) {
			throw new IllegalArgumentException("The provided ip is not valid: "
					+ host[1]);
		}
		...
		if (port <= 0 && port >= (1 << 16)) {
			throw new IllegalArgumentException(
					"The provided port is outside the valid range: " + port);
		}
	...
	// The options that were chosen by the user (one byte). Currently only the
	// lsb has a meaning: User is behind a ssl-proxy (yes:1; no:0)
	private int options;
	...
	/**
	 * Under certain circumstances the client sends duplicate
	 * CertVerifyRequest-messages. Therefore CertVerifyResults are cached and
	 * resent on duplicate CertVerifyRequest-messages. The KEY of the
	 * CertVerifyResultCache-table is the hash of the CertVerifyRequest. This
	 * hash is calculated here.
	 * 
	 ...
	 */
	public byte[] getHash() throws MessageSerializationException, IOException,
	...
	/**
	 * @param hostIP
	 *            The IP of the Host from which the certificate has been
	 *            received
	 */
	...
	/**
	 * @param hostName
	 *            The name of the Host from which the certificate has been
	 *            received
	 */
	...
	/**
	 * @param hostPort
	 *            The port of the Host from which the certificate has been
	 *            received
	 */
	...
	/**
	 * @param options
	 *            The options that were chosen by the user (one byte)
	 */
	...
	public void writeContent(OutputStream out)
		...
		try {
			...
			// Second part: the number of how many certificates are part of the
			// chain
			out.write(this.certChain.length & 255);

			// Third part: the certificate chain (beginning with the server
			// certificate)
			...
			out.write(new String(hostName + "|" + hostIP.getHostAddress() + "|"
					+ String.valueOf(hostPort)).getBytes());
		}

		catch (IOException e) {
			throw new MessageSerializationException(
					"Could not serialize certificate verification request", e);
